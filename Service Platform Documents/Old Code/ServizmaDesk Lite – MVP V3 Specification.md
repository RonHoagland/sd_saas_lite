...
## Pricing
- $28 per seat per month ($24/month billed annually)
- Billed monthly or annually per active seat
...
ServizmaDesk is composed of two separate and independent Django applications, each with its own PostgreSQL database. Both databases may reside on the same PostgreSQL server but are fully isolated from each other.

### ServizmaDesk Platform (SDP)
The Platform is a separate internal Django application used to operate the ServizmaDesk business itself. It is the gatekeeper for all customer accounts.
...
- Communications (transactional email to ServizmaDesk customers)

SDP is a prerequisite to building the ServizmaDesk Tenant App. It must be specified and built before tenant provisioning can function.

> **Note:** A full specification for ServizmaDesk Platform is a separate document and is a dependency of this project. It must be completed before SDTA tenant provisioning can be implemented.

### ServizmaDesk Tenant App (SDTA)
...
SDTA responsibilities:
- All operational functionality for Lite and Plus
- Tenant-scoped data storage and access
- Plan-aware feature enforcement (reads plan status from SDP)
- Session and audit tracking

### Communication Between SDP and SDTA
At the Lite stage, SDTA will communicate with SDP strictly via an internal REST API. Direct database reads are strictly prohibited to ensure proper data isolation and maintain the integrity of the SDP database.

> **Decision:** Internal REST API from Day 1. SDTA will query SDP dynamically to enforce plan limits, status, and authorized features.

### Future Tiers (Pro / Enterprise)
Pro and Enterprise architecture is explicitly deferred. The decision to extend SDTA or create a separate application will be made based on product requirements at that time. No architectural assumptions are being made in this document.
...
### Strategy
SDTA uses a **shared database, shared schema** multi-tenancy model. All tenants share the same PostgreSQL database and the same set of tables. Every table includes a `tenant_id` field that scopes all data to a specific tenant account.
...
### Tenant ID
- Every tenant is assigned a **UUID** generated by ServizmaDesk Platform at account provisioning
- UUIDs are used (not sequential integers) to prevent enumeration or guessing
- The `tenant_id` UUID is passed to SDTA at provisioning and stored in the tenant account record

### Database-Level Enforcement
Every table in SDTA (with the exception of system-level configuration tables) includes:
...
> **Decision:** Three-layer defense. Database field constraint → Django middleware → PostgreSQL RLS. All three layers must fail simultaneously for a cross-tenant data leak to occur. This is the required architecture for SDTA.
...
### Tenant Provisioning
When a new customer account is created in SDP, the provisioning process:
1. Generates a tenant UUID in SDP
2. Creates the tenant record in SDTA
3. Creates the first Administrator Employee record in SDTA
4. Creates the default Preferences record in SDTA
