# ServizDesk Invoice Calculation Specification V1
...
4.  **`amount_paid`**: Denormalized sum of all linked `Payment` records applied to this invoice. Initialized to `0.00`. Must be incremented when a payment is applied and decremented when a payment is voided.
5.  **`balance_due`**: Calculated as `invoice_total − deposit_applied − amount_paid`. Initialized to `invoice_total` (minus any quote deposit). Must be recalculated whenever `invoice_total`, `deposit_applied`, or `amount_paid` changes.
