# ServizDesk
# ServizDesk Lite – MVP V4 Specification (Working Draft)
...
Lite tier pricing is defined in **ServizDesk Pricing & Billing Specification V2, Section 3.1**.
...
Storage upgrade pricing is defined in **ServizDesk Pricing & Billing Specification V2, Section 4**.
...
ServizDesk Lite is built on the foundation defined in **ServizDesk Technical Architecture V2**.
...
- Django 6.x backend with HTMX v2.x frontend
...
- **ServizDesk Platform (SDP):** Back-office system handling signup, billing, and provisioning
- **ServizDesk Tenant App (SDTA):** Customer-facing application for Lite/Plus operations

For complete architectural details, see:
→ **ServizDesk Technical Architecture V2**

For complete data model specifications, see:
→ **ServizDesk Technical Architecture V2**
→ **ServizDesk Data Models V6**
...
New customers sign up through the ServizDesk marketing/signup site. The signup flow is fully self-service — no manual intervention by ServizDesk staff is required for a standard signup.
...
1. Administrator contacts ServizDesk support (via SDP support channel)
2. ServizDesk support staff initiates a recovery request in SDP
...
4. Once identity is verified by ServizDesk support staff in SDP, the Administrator account is unlocked in SDTA
...
### Governance Rules
- ServizDesk support staff cannot unlock an Administrator account without passing identity verification
...
4. Locked Administrator accounts are unlocked only through SDP identity verification by ServizDesk support staff.
...
Record numbers serve two purposes: they give users a short, memorable reference for every record, and they give ServizDesk support staff a human-readable identifier when assisting customers.
...
Each tenant maintains their own Stripe account. Money flows directly from the tenant's customers into the tenant's Stripe account. ServizDesk never handles tenant funds.
...
2. Each tenant connects their own Stripe account — ServizDesk never handles tenant funds.
...
The detailed onboarding content, copy, and marketing experience is defined in the ServizDesk Platform specification. This section defines only the SDTA-side behavior.
...
- ServizDesk Platform specification (separate document — required dependency)
...
8. ServizDesk Platform can provision a new tenant and initialize a SDTA account atomically.
