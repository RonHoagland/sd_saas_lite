# ServizDesk Onboarding Checklist Specification (SDTA)
**Document Version:** V2
...
- **Trigger**: At least one record exists in the `InventoryItem` table for this `tenant_id`.
- **Note**: The internal model name is `InventoryItem`. On Lite tier the UI labels this "Product"; on Plus/Pro/Enterprise it is "Inventory Item."
...
- **Triggers**: Whenever a write operation occurs on `TenantPreference`, `Customer`, `Asset`, `InventoryItem`, `StripeConnection`, or `User`, a background signal (e.g., Django `post_save`) MUST evaluate the corresponding logic and update the `checklist_items` JSONField in the `OnboardingState` record.
