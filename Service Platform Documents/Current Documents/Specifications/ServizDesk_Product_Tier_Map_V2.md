# ServizDesk — ServizDesk Product Tier Map
...
This document is the first of three master project documents that govern the design, architecture, and construction of the ServizDesk product family. It defines what is being built at each product tier and establishes the boundaries that all subsequent technical and build decisions must respect.
...
This document supersedes all prior tier, feature, and pricing references found in legacy ServizDesk/ServizDesk documentation and earlier ServizDesk draft documents. Where conflicts exist between this document and any other source, this document governs.
...
- **Company Name:** ServizDesk LLC
- **Structure:** Minnesota Multi-Member LLC
- **Operations:** US-based (founder relocating to Philippines, Year 2)

## 2.2 Product Family

- **Product Name:** ServizDesk
- **Product Type:** Multi-tenant SaaS field service management platform
- **Platform Name:** ServizDesk Platform (SDP) — internal operations and billing platform
- **Application Name:** ServizDesk Tenant App (SDTA) — customer-facing product
...
All pricing, billing cycles, storage add-ons, and Founding Partner program details are defined in the **ServizDesk Pricing & Billing Specification V2**.
...
ServizDesk offers a 14-day free trial with no credit card required. Full trial terms, expiration flow, and conversion process are defined in **ServizDesk Pricing & Billing Specification V2, Section 6**.
...
→ **ServizDesk Pricing & Billing Specification V2, Section 6**

---

# 5. Founding Partner Program

ServizDesk offers two Founding Partner tiers with exclusive legacy pricing for early adopters:
...
→ **ServizDesk Pricing & Billing Specification V2, Section 5**
...
Customer → Asset → Quote → Work Order → Invoice → Payment (manual recording)
...
- **Customers:** Create, edit, delete with status lifecycle. Embedded contacts (2), addresses (billing required, shipping optional), phone numbers (fax, phone1, phone2), social info. Customer-only — no vendor capability. Linked to all operational modules.
...
- **Assets:** The core differentiator. Customer-linked assets with make/model, serial number, install date, warranty start/end, status. Full service history visible (quotes, work orders, invoices). Documents and notes attachable. CSV export.
- **Quotes:** With line items. Status lifecycle (Draft, Sent, Viewed, Accepted, Rejected, Expired, Converted). Linkable to work orders for conversion.
- **Work Orders:** With line items. Date and optional time. Status lifecycle (Draft, Scheduled, In Progress, On Hold, Completed, Closed, Cancelled). Maintenance flag for manual tagging. Assignable to employees. Linkable to invoices.
- **Invoices:** With line items. Auto-calculated subtotal, tax, total. Status lifecycle (Draft, Issued, Viewed, Partially Paid, Paid, Overdue, Void, Written Off). Auto-lock when Issued; permanently locked when Paid.
...
- **Documents:** Attachments on all major objects. 3 GB storage included; add-ons available: +3 GB (total 6 GB) or +4 GB (total 10 GB max). See Pricing & Billing Specification V2 Section 11.1 for add-on pricing. Max 10 attachments per record. Uploads blocked at 100% storage.
...
- **Preferences:** Tenant info, tax rate, currency, date format, timezone, basic configuration.
...
- CSV export from all list views (restricted to Administrator role).
...
- **SMS:** 100 points/month. Manual sends only — no automated triggers.
- **System Email:** 400 email points/month. Manual sends only (quote sends, invoice sends, manual customer messages). No automated triggers. Sent via ServizDesk's platform domain with the tenant's company name as display sender. See Pricing & Billing Specification V2, Section 10 and 10A.

### Explicitly Not Included in Lite

- Automation of any kind
- Automated SMS/email triggers (appointment reminders, follow-ups, payment reminders)
...
- **Vendors:** Standalone entity — completely separate from Customers. Vendor records have their own Contacts, Phone Numbers, and Addresses (via shared Triad tables). No connection to Customer records. Vendor capability is not available in Lite.
...
- **Maintenance Scheduling:** User selects an Asset, duration (1–5 years), and frequency (yearly, monthly, bi-weekly, etc.). System auto-generates all work orders for the schedule in Scheduled status. Individual generated work orders are fully editable and deletable.
...
- **SMS — Automated Triggers + Increased Allotment:** Adds automated SMS triggers (appointment reminders, on-my-way notifications, post-job follow-ups, payment reminders). Higher monthly point allotment than Lite. Additional point tiers available for purchase.
- **System Email — Automated Triggers + Increased Allotment:** Adds automated email triggers. Higher monthly email point allotment than Lite. Overage charged per point. Default mode sends from ServizDesk's platform domain with the tenant's company name as the display sender. Custom Domain authentication (emails from tenant's own domain) is available as a paid add-on on Pro and Enterprise — see Pricing & Billing Specification V2, Section 11.4. BYOS (Bring Your Own SMTP) is not supported. See ServizDesk Email Specification V1 for full architecture.
- **PDF Generation:** Available for Quotes, Invoices, Work Orders, and Payment receipts. No quantity limit.
- **Advanced Payment Processing:** Tenant unlocks deep embedded payment APIs (custom card forms, split-payments, recurring subscriptions) rather than just the standard Stripe Payment Links provided in Lite. ServizDesk never holds card data.
...
- **Fleet Maintenance Add-On (Available from Plus):** Optional paid add-on for service businesses that operate a vehicle fleet. Adds vehicle records, maintenance logging, mileage tracking, and mobile parts stock. Priced at $15/vehicle/month. Available on Plus, Pro, and Enterprise. Full details in **ServizDesk Pricing & Billing Specification V2, Section 12** and **ServizDesk Top-Down Specifications V4, Section Fleet Maintenance**.
...
- **System Email — Increased Allotment:** Higher monthly email point allotment than Plus. Same overage rate. Custom Domain add-on available at Pro tier.
...
## 6.4 ServizDesk Enterprise

Enterprise is the long-term vision for ServizDesk as a full-featured ERP for larger service organizations. Enterprise is not in active development and will not enter planning until Pro is stable and generating revenue. The following represents the directional scope and is subject to change.
...
| Add-On | Lite | Plus | Pro | Entp |
|---|:---:|:---:|:---:|:---:|
| Fleet Maintenance ($15/vehicle/month) | — | + | + | + |
| Extra Storage (+3 GB or +4 GB options) | + | + | + | + |
| Custom Domain Email (paid add-on) | — | — | + | + |

*A `+` indicates the add-on is available for purchase at that tier. `—` indicates the add-on is not available.*
...
- **ServizDesk Platform Database (SDP):** Holds all account-level data: tenant records, plan status, billing history, security credentials, staff accounts, audit logs. Operated by ServizDesk internally.
- **ServizDesk Application Database (SDTA):** Holds all tenant operational data: companies, assets, quotes, work orders, invoices, payments, notes, documents, tasks, and all other business records. Shared by all tenants across all tiers.
...
Communication between SDP and SDTA occurs exclusively via the **Internal REST API**. SDTA authenticates to SDP to retrieve or verify tenant plan status, seat limits, and add-on allocations. Direct database reads across applications are strictly prohibited. This API-driven approach ensures application isolation even when components are deployed on separate infrastructure.
...
Customers, People, Products, Assets, Notes, Documents, and Tasks are foundation objects that exist across all tiers. All business modules (procurement, CRM, scheduling, etc.) attach to these foundation objects. New modules never recreate foundation functionality.
...
### 9.5 Vendors Are a Standalone Entity

Vendors are a completely separate entity from Customers. There is no `customer_type` flag, no shared table, and no connection between the two. Vendors have their own Contact, Phone, and Address records via the shared Triad tables. In Lite, there is no reference to Vendors in any way — the Vendor entity and all vendor-related UI are gated to Plus and above.
...
ServizDesk never stores raw payment card data at any layer. All card collection uses Stripe Checkout (for ServizDesk billing) or Stripe Connect (for tenant payment processing). This is a non-negotiable security and compliance requirement.
...
- Stripe SDK version
- Transactional email provider selection (for ServizDesk-to-tenant communications from SDP)
- System email architecture for tenant-to-customer communications (ServizDesk-managed mode details)
...
- All tier and feature references in ServizDesk/ServizDesk legacy documentation
- All prior ServizDesk SaaS Operational Plan versions (pricing and tier structure sections only — financial projections and business planning sections remain valid in the current operational plan)
- ServizDesk Product Structure High-Level V5 (replaced by the tier definitions and feature matrix in this document)

### This Document Depends On

- ServizDesk Platform (SDP) V2 Specification — for platform architecture, provisioning, and billing details
- ServizDesk Lite MVP V4 Specification — for detailed Lite module specifications
- ServizDesk Pricing & Billing Specification V2 — for all pricing, trial, storage, and Founding Partner details

### Legacy Reference Documents (Not Superseded, Used for Reference)

The following ServizDesk/ServizDesk documents contain module-level detail that remains largely applicable to the SaaS version. They should be consulted during module development but must be reconciled with the SaaS architecture, multi-tenancy model, and tier boundaries defined in this document and Document 2.

- ServizDesk Customers MVP Functionality
- ServizDesk Products MVP Functionality
- ServizDesk Quotes MVP Functionality
- ServizDesk Work Orders MVP Functionality
- ServizDesk Invoices MVP Functionality
- ServizDesk Payments MVP Functionality
- ServizDesk Simple Ledger MVP Functionality
- ServizDesk Notes MVP Functionality
- ServizDesk Documents MVP Functionality
- ServizDesk Leads MVP Functionality
- ServizDesk Assets MVP Functionality
- ServizDesk Preferences Functionality
- ServizDesk Reports Functionality
- ServizDesk Admin Access MVP Functionality
- ServizDesk User Management
- ServizDesk Sequence Management System
- ServizDesk Service Offerings
