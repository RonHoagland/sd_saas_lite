...
**Owner:** ServizDesk  
**Supersedes:** All pricing sections in Product Tier Map V1, Product Tier Map V2, Platform (SDP) Specification V1, Platform (SDP) Specification V2, Operational Plan V2, Operational Plan V3, and Top-Down Specifications V1/V3/V4

---

# 1. Document Purpose & Scope

This document is the single source of truth for all ServizDesk pricing, billing, and Founding Partner program details. All other ServizDesk specifications reference this document for pricing information.
...
ServizDesk pricing is designed to:
...
### ServizDesk Positioning

ServizDesk Lite at $29/month (annual) undercuts both primary competitors while delivering superior asset-centric architecture:
...
ServizDesk uses a **gateway → revenue → growth** tier model:
...
**Pricing Note:** Lite is priced at $29/month (annual) and $35/month (monthly). At $29/month annual, ServizDesk Lite is:
- 63% cheaper than HousecallPro Basic ($79)
- 26% cheaper than Jobber Core ($39)
- Positioned as "inexpensive" rather than "cheap" — deliberate signal of quality

**Enterprise Pricing Note:** Enterprise is priced at $179/month (annual) and $216/month (monthly). This positions ServizDesk Enterprise as dramatically more accessible than ServiceTitan ($200–$500/technician/month with $5K–$50K+ implementation fees) while delivering equivalent or superior capability — multi-location, full ERP accounting, route optimization, and bank feed integration — with no implementation fees and transparent published pricing.
...
ServizDesk uses **DigitalOcean Spaces** as its file storage provider. DigitalOcean Spaces is an S3-compatible object storage service with a built-in CDN. It is the designated storage backend for all tenant-uploaded files including job photos, documents, attachments, and exported reports.
...
These are ServizDesk's infrastructure costs. Tenants are not billed at DO Spaces rates — they are billed at the storage add-on rates defined in Section 11.1, which are priced to cover platform cost plus margin.
...
The Founding Partner program offers exclusive legacy pricing to the first 20 companies (10 Lite, 10 Plus) who commit to ServizDesk during the initial launch phase. This program serves three strategic purposes:
...
> - You have 12 active seats. ServizDesk Lite allows a maximum of 10 seats. Please set 2 employees to Terminated status.
> - You are using 15 GB of storage. ServizDesk Lite allows a maximum of 10 GB. Please reduce storage usage before downgrading."
...
**Customer-Initiated:**
- Customer may cancel from Billing section in SDTA or by contacting ServizDesk support
- Cancellation takes effect at the end of the current billing period
- No partial refunds for unused time in current period
- Customer retains full access until end of paid period

**Staff-Initiated:**
- ServizDesk staff may cancel account for Terms of Service violations or non-payment
...
- Account status: `Cancelled (Read Only)` (TenantState enum value)
...
- Account status: `Cancelled` (TenantState enum value)
...
**Critical Security Requirement:** ServizDesk never stores raw payment card data at any layer.

**Platform Billing (ServizDesk subscription charges):**
...
- **No card numbers, CVV, or expiration dates stored in ServizDesk systems**
...
ServizDesk qualifies for **PCI SAQ A** (Self-Assessment Questionnaire A) because:
- No card data flows through ServizDesk systems
- No card data is stored on ServizDesk servers
...
**Applies to:** All tiers — Lite, Plus, Pro, and Enterprise
...
**Level 2: Stripe Connect Platform Fees (Paid by ServizDesk)**
...
- These are the fees Stripe charges **ServizDesk** for providing the Connect infrastructure
...
- Stripe charges ServizDesk: $2 (monthly active fee) + $0.25 + $24.27 (0.25% of $9,709.70) = **$26.52**
- **Total to Stripe: $316.82** (2.9% from tenant + platform fees from ServizDesk)

### Industry Standard: Application Fees

Most FSM SaaS platforms (Jobber, HousecallPro, FieldPulse) charge tenants an **"application fee"** of 1-2% on top of the standard Stripe processing fees to cover Connect costs and generate profit. ServizDesk applies the same model across all tiers, consistent with industry practice, at a rate significantly below competitors.
...
### ✅ Locked Decision: ServizDesk Application Fee — 0.5%
...
**Example: $10,000 tenant transaction with ServizDesk**
- Customer pays $10,000
- Stripe deducts: $290.30 (2.9% + $0.30) — paid by tenant
- ServizDesk application fee: $50.00 (0.5%) — paid by tenant
- Tenant receives: $9,659.70
- ServizDesk pays Stripe Connect fees: ~$26.52
- **ServizDesk net: ~$23.48**
...
| **ServizDesk** | 2.9% + $0.30 | 0.5% | $340.30 |
| **Savings with ServizDesk** | — | — | **$100** |
...
- Itemized on payout statements: "Stripe Processing: $290.30 | ServizDesk Platform Fee: $50.00"
...
**Reward:** A subscription credit representing **0.1%** of their total processed volume for that year (effectively a 20% rebate on the 0.5% application fee they paid). ServizDesk retains the remaining 0.4%. 
...
2. ServizDesk's application fee (0.5%)
...
- ServizDesk registers as the ISV brand; each tenant is registered as a campaign under the ServizDesk A2P umbrella
- A2P campaign registration fees (~$1.50–$10/month per active SMS tenant) are a platform operating cost absorbed by ServizDesk — not passed through to tenants
...
Once usage patterns are established post-launch, ServizDesk intends to offer optional SMS point packages as flat-fee monthly add-ons. Point packages allow tenants who consistently exceed their included allocation to replace variable overage billing with predictable flat-fee billing.
...
ServizDesk maintains a single Postmark account. All tenant emails across all tiers flow through this account. Postmark has no concept of individual tenants — ServizDesk meters per-tenant usage internally via application-level counters, resetting at each tenant's billing anniversary.
...
**Sending modes:**
- **Platform-managed (default):** ServizDesk sends from its own domain (`notifications@mail.servizdesk.com`). Tenant's company name appears as the display sender name. Available on all tiers. No additional cost.
- **Custom domain (paid add-on):** Tenant authenticates their own domain via DNS records (SPF, DKIM, DMARC). Emails send from the tenant's domain through Postmark's infrastructure. Available on Pro and Enterprise only. See Section 11.4.
- **BYOS (Bring Your Own SMTP):** Not supported. ServizDesk does not allow tenants to route platform email through their own SMTP credentials. This decision was made to maintain delivery visibility, metering accuracy, and consistent tenant experience. Competitors (Jobber, HousecallPro, FieldPulse, Workiz) do not offer BYOS either.

## 10A.3 Competitive Baseline — What Email Must Do

ServizDesk's platform-managed email must match the following competitor baseline at all relevant tiers:
...
No competitor captures inbound email replies back into their platform. ServizDesk's inbound reply processing (Phase 1 add-on) is above-market functionality.
...
### 10A.5.1 Email Overage Billing Lifecycle

Email overage is billed on the tenant's billing anniversary. The `reset_email_usage_counters` background task (see Background Tasks Specification V2, Section 3.2) reads the accumulated `email_points_overage` counter before resetting it. If overage > 0, a Stripe invoice line item is created for the overage amount at the per-point overage rate defined in this specification. The overage charge appears on the tenant's next invoice cycle. Counter is then reset to 0 for the new billing period.
...
Once usage patterns are established post-launch, ServizDesk intends to offer optional email point packages as flat-fee monthly add-ons — mirroring the SMS point package roadmap.
...
*Enterprise tenants requiring more than 1,500 GB should contact ServizDesk for custom storage pricing.*
...
GPS Tracking is not part of the current ServizDesk add-on roadmap. There are no current plans to build or offer a GPS Tracking module.

If GPS Tracking is pursued in a future phase, the intended approach is a full platform partnership (Option A) — reselling or white-labeling an established GPS telematics provider (e.g., Samsara, Motive, or Azuga) via an ISV arrangement, with ServizDesk surfacing location data natively within the platform. This is a hardware-dependent feature that requires significant commercial and integration investment and is not appropriate for the current product stage.

A GPS Tracking feasibility and strategy brief has been prepared as a separate document: **ServizDesk GPS Tracking Strategy & Feasibility Brief**. All market research, cost analysis, pricing considerations, and open decisions related to GPS Tracking are maintained there.

## 11.4 Custom Domain Email Add-On

The Custom Domain Email add-on enables tenants to send platform emails from their own business domain and receive customer email replies back into ServizDesk. This is a Phase 1 premium feature — no competitor offers equivalent functionality at any price point.

| Detail | Pro | Enterprise |
|---|---|---|
| **Price** | $20/month per tenant | $40/month per tenant |
| **Available To** | Pro only | Enterprise only |
| **Billing Model** | Flat per-tenant/month | Flat per-tenant/month |

> **⚠️ Pricing Clarification Note:** Original table had "Pro, Enterprise" in the Pro column's "Available To" row, which was ambiguous (it implied Enterprise tenants could access the $20/month rate). Corrected to reflect the intended per-tier pricing: Pro tenants pay $20/month, Enterprise tenants pay $40/month. Verify this is the intended pricing before implementation.

**Phase 1 includes:**

- **Custom domain outbound:** Tenant provides their domain (e.g., `acmehvac.com`). ServizDesk generates the required SPF, DKIM, and DMARC DNS records and displays them with setup instructions. Once the tenant adds the records and domain verification is confirmed via Postmark, all platform-managed emails for that tenant send from their domain (e.g., `From: quotes@acmehvac.com`) through Postmark's infrastructure
- **Inbound reply processing:** ServizDesk embeds a unique reply address token in the `Reply-To` header of every outbound email. When a customer replies, Postmark receives the reply and fires a webhook to ServizDesk. The inbound handler identifies the tenant and the originating record (Quote, Invoice, Work Order, Customer) from the token and attaches the reply as a communication thread entry visible to all authorised team members
...
- Domain DNS records remain the tenant's responsibility — ServizDesk provides setup instructions but does not manage the tenant's DNS
...
- **ServizDesk Product Tier Map V1 and V2** — Section 3 (Pricing and Billing), Section 4 (Free Trial Structure), Section 5 (Founding Partner Program)
- **ServizDesk Platform (SDP) Specification V1 and V2** — Section 5.2 (Plan Limits Table), Section 5.3 (Billing Cycles), Section 6.3 (Billing Model), Section 6.4 (Seat Billing), Section 6.5 (Storage Add-On Billing)
- **ServizDesk SaaS Operational Plan V2** — Section 2 (Product Tier Structure), Section 3 (Founding Partner Program)
- **ServizDesk SaaS Operational Plan V3** — any pricing sections
- **ServizDesk Top-Down Specifications V3 and V4** — Section 16 Pricing rows in the Tier Feature Mapping table

## 13.2 This Document Depends On

- **ServizDesk Product Tier Map V2** — for feature definitions by tier
- **ServizDesk Platform (SDP) Specification V2** — for Stripe technical implementation, provisioning flows, and billing automation
- **ServizDesk Top-Down Specifications V4** — for entity definitions and module scope

## 13.3 Documents That Reference This Specification

- **ServizDesk Product Tier Map V2** — references this document for all pricing details
- **ServizDesk Platform (SDP) Specification V2** — references this document for billing rules and pricing structure
- **ServizDesk Lite MVP V4 Specification** — references this document for Lite pricing and trial structure
- **ServizDesk Top-Down Specifications V4** — references this document for all pricing; contains no pricing data of its own
- **ServizDesk Email Specification V1** — Section 11.4 of this document defines Custom Domain Email add-on pricing referenced in the Email Spec
- **ServizDesk GPS Tracking Strategy & Feasibility Brief V1** — references this document for GPS future consideration note
- **ServizDesk Plus Specification (future)** — references this document for Plus pricing and SMS/email point pricing
- **ServizDesk Pro Specification (future)** — references this document for Pro pricing
- **ServizDesk Enterprise Specification (future)** — references this document for Enterprise pricing

---

# 14. Competitive Positioning Summary

## 14.1 Why ServizDesk Pricing Works
...
- No multi-year lock-in — ServizDesk publishes pricing and lets tenants cancel
