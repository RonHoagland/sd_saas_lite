# ServizDesk Email Specification V1
...
This document is the authoritative specification for all email sending, receiving, and routing functionality in ServizDesk. It covers:
...
**Pricing** is maintained exclusively in the **ServizDesk Pricing & Billing Specification V2, Section 10A and Section 11.4**. This document contains no pricing data.
...
## 2.2 ServizDesk Baseline Requirements

To match competitors at all relevant tiers, ServizDesk must provide:
...
## 2.3 ServizDesk Differentiation

ServizDesk exceeds the competitor baseline with:
...
ServizDesk uses **Postmark** as its transactional email delivery platform. Postmark was selected for:
...
- Already in the ServizDesk confirmed tech stack

## 3.2 Single Account / Multi-Tenant Model

ServizDesk operates **one Postmark account**. All tenant emails across all tiers flow through this single account. Postmark has no awareness of individual tenants — ServizDesk manages all tenant-level metering internally.

**How per-tenant metering works:**
- Every outbound email dispatched through Postmark is logged against the originating tenant's `email_points_used` counter in the ServizDesk database
...
Postmark uses Message Streams to separate email types. ServizDesk should configure at minimum:
...
All emails send from ServizDesk's verified sending domain. Postmark handles delivery, bounce management, and reputation maintenance.

- **From address:** `notifications@mail.servizdesk.com` (or similar verified subdomain)
...
### Mode 2: Custom Domain (Paid Add-On — Pro and Above)
...
1. **No delivery visibility:** ServizDesk cannot track delivery status, bounces, or open rates through the tenant's own mail server
2. **No metering accuracy:** Email point counting requires ServizDesk to control the sending path
3. **Support liability:** Gmail, Hotmail, and similar consumer accounts have strict daily sending limits (Gmail: 500/day free, 2,000/day Workspace). When limits are hit, emails silently fail. Tenants blame ServizDesk. This creates unresolvable support burden
...
### Lite — No System-Originated Customer Emails

**Lite tenants have ZERO system-originated emails to their customers.** The system does not send Quote deliveries, Invoice deliveries, reminders, "on my way" notifications, or any other customer-facing messages. All customer-facing communication in Lite is handled by the tenant's user through their own email client (Outlook, Gmail, Apple Mail, etc.). The in-app "Send" action on a Quote or Invoice is a state transition that triggers snapshot and lock (per Lite UI Spec §18, §20, §26); it does NOT originate an email. Users obtain the document via browser print and send it themselves.

Consistent with SMS manual-only restriction and with the Lite MVP V4 Specification's exclusion of "System email sending" and "Transactional email provider integration" from customer-facing messaging.

**Platform-level emails (not customer-facing) that DO fire in Lite** are limited to:

| Email Type | Trigger | Notes |
|---|---|---|
| Tenant welcome / provisioning | Auto — on tenant signup (SDP-originated) | Sent to tenant admin, not to end customers |
| Administrator password reset | Manual — from Admin Area | Admin self-service email link. Non-admin users in Lite do NOT get self-service reset (see Lite MVP V4 password reset flows). |

These platform emails use Postmark under the ServizDesk account like all tiers; they are SDP-level concerns, not tenant-driven customer messaging.
...
2. **Inbound reply processing** — customer replies are captured back into ServizDesk and attached to records
...
3. ServizDesk generates and displays three DNS records:
...
5. Tenant clicks "Verify Domain" — ServizDesk triggers Postmark domain verification
...
ServizDesk manages the Postmark domain registration on the backend. The tenant only fills in a form and adds DNS records — no Postmark account required.

## 5.3 Inbound Reply Processing Setup

Once the domain is verified, ServizDesk automatically configures inbound reply processing:

1. ServizDesk generates a unique inbound address per tenant: `reply+[unique_tenant_token]@inbound.servizdesk.com`
2. This address is embedded as the `Reply-To` header on every outbound email for that tenant
3. When a customer replies, Postmark receives the email at the inbound address and fires a webhook to ServizDesk
4. ServizDesk's inbound webhook handler:
...
**Result:** The tenant sees a full conversation thread on the Quote or Invoice record — outbound send, customer reply, any subsequent replies — without leaving ServizDesk.
...
Tenants on Lite or those who have not purchased the Custom Domain add-on use platform-managed mode by default (emails from `@mail.servizdesk.com`). This matches the functionality provided by all competitors and is fully acceptable for entry-level tenants.
...
Phase 2 extends inbound email processing from simple reply capture to a configurable routing system. Tenants can define multiple inbound email addresses, each mapped to a specific action within ServizDesk.
...
| Address | Action |
|---|---|
| `inquire@[tenant-domain]` | Create new Lead in ServizDesk CRM, notify assigned sales contact, appear in Lead queue |
...
| `service@[tenant-domain]` | Create new Service Request / service request from recognised customer |

**Unrecognised sender handling (Phase 2):**
- Email arrives from address not in the customer database
- ServizDesk creates a new Lead record with the sender's email, name (from email display name), and message body
...
1. **Catch-all vs. specific addresses:** Does ServizDesk configure a catch-all (`*@[tenant-domain]`) and route based on To address, or register specific addresses only? Catch-all is simpler but requires handling spam/noise. Specific addresses require the tenant to update DNS for each new address.
...
4. **Auto-responder:** Should ServizDesk send an automatic acknowledgement when an inbound email creates a record? (e.g., "Thanks, we received your inquiry and will be in touch.") This would consume an outbound email point.
...
Per-tenant email counters in the ServizDesk database:
...
2. ServizDesk checks `email_points_used` vs `email_points_included`
...
| # | Decision | Notes |
|---|---|---|
| 1 | ~~Plus tier access to custom domain outbound only~~ | **✅ RESOLVED — March 2026.** Custom Domain Email remains **Pro and Enterprise only**. Plus tier does NOT get access. All referencing documents (Technical Architecture V2, Top-Down V4, Pricing V2, Product Tier Map V2) already reflect this as "Pro and above" — no updates required. |
...
| 4 | ~~Email retention in Communications Log~~ | **✅ RESOLVED — March 2026.** 12-month retention. Background Tasks Specification V2 (`purge_email_delivery_logs`) and Database Specification V2 Section 10.1 both confirm 12-month rolling retention for email delivery logs, which exceeds the Postmark 45-day baseline. |
...
**Note:** All 8 pending updates were completed as of March 2026. Version references below reflect the document version at time of original authoring; the current version is Top-Down Specifications V4.

| # | Document | Location | Issue | Required Change | Status |
|---|---|---|---|---|---|
| 1 | **Top-Down Specifications V1** | Section 10.6 (Email SMTP Configuration) | Entire section defines BYOS SMTP credential fields (Host, Port, Username, Password, TLS, SSL) | Replace with Custom Domain Authentication section — tenant provides domain, ServizDesk generates DNS records (SPF, DKIM, DMARC), domain verified via Postmark | ✅ Complete — V1 updated March 2026 |
...
| 7 | **Technical Architecture V2** | Line ~193 | States "Two modes are planned: ServizDesk-managed and Bring Your Own SMTP (BYOSMTP)" | Update to confirmed architecture: Mode 1 = platform-managed (default, all tiers via Postmark); Mode 2 = Custom Domain (paid add-on, Pro+, DNS-authenticated); BYOS dropped | ✅ Complete — V2 updated March 2026 (also fixed stale SMS allotments 100/250→350/750, confirmed Twilio as provider dropping Plivo candidate, removed stale system email deferral from Defers To) |
...
- **Top-Down Specifications V4** — Communication features per tier (Section 14.5)
...
- ~~ServizDesk GPS Tracking Strategy & Feasibility Brief V1~~ — archived; removed from working specification set
