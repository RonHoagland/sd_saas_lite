# ServizDesk
# ServizDesk Platform (SDP) – V2 Specification

> **Note:** SDP stands for ServizDesk Platform. It is the central administrative and operational back-office system for the ServizDesk ecosystem. (Working Draft)

> **Document Version:** V2
> **Status:** Working Draft — Outline Only
> **Last Updated:** March 2026
> **Compatibility:** Designed to synchronize with ServizDesk Lite MVP V4 Specification and Product Tier Map V2.
...
The ServizDesk Platform (SDP) is the central control system for all ServizDesk products. It is the authoritative back office that governs every customer account, billing relationship, and security credential across the entire ServizDesk product family.

SDP is not a customer-facing product in the traditional sense. It is an internal operations platform used by ServizDesk staff, with a limited self-service surface exposed to customers for signup, billing, and account management.

No ServizDesk tenant account can exist without SDP creating it first. No charge is made to a customer without SDP initiating it. No Administrator credential is stored anywhere except SDP.

---

## 1.2 Core Roles

SDP serves five distinct roles within the ServizDesk business:

**Gatekeeper**
No tenant account exists in any ServizDesk application without SDP provisioning it first. SDP generates the Tenant UUID, initializes the account in the target application, and controls whether that account remains active or suspended.

**Single Source of Truth**
SDP holds the authoritative record for every customer account across all ServizDesk products — company details, plan status, seat counts, storage limits, and account history. The ServizDesk applications read from SDP but do not own this data.

**Billing System**
All customer charges flow through SDP via ServizDesk's Stripe account. Subscription billing, seat changes, storage add-ons, proration, and payment failure handling are all managed here.

**Security Authority**
Administrator security credentials — Security PIN and security question answers — are stored exclusively in SDP and are never replicated to any ServizDesk application. Account recovery for locked Administrators is initiated and completed through SDP.

**Staff Operations Hub**
ServizDesk support and operations staff perform all account management tasks through SDP — account lookup, plan changes, seat adjustments, account unlocks, suspensions, and cancellations.

---

## 1.3 ServizDesk Tenant Apps Served

SDP is the platform for all current and future ServizDesk products:

| Application       | Status         |
|-------------------|----------------|
| ServizDesk Lite      | In development |
| ServizDesk Plus      | Planned        |
| ServizDesk Pro       | Planned        |
| ServizDesk Enterprise| Future — scope not yet defined |

All ServizDesk applications share the same SDP instance. Each application reads plan and account status from SDP at runtime.
...
- **Application-agnostic** — SDP is designed to serve all current and future ServizDesk products without structural changes
...
- SDP does not store the day-to-day operational business data of tenants — things like customer records, work history, invoices, and payments live exclusively in the ServizDesk applications
...
- SDP does not handle payments between tenants and their customers — that is managed through the tenant's own Stripe account within the ServizDesk application they are using
...
SDP is a Django application backed by a PostgreSQL database. It shares the same server as the ServizDesk Tenant App (SDTA) but maintains a fully separate database. The two applications are independent — neither can write to the other's database.
...
- ServizDesk staff user accounts
- SDP-side audit log
...
- ServizDesk staff authenticate using staff credentials managed within SDP
...
Customers interact with ServizDesk through two touchpoints:

1. **Signup flow** — before the customer has a ServizDesk account, hosted by SDP
2. **Billing section within SDTA** — after signup, the customer manages their account and billing from inside their ServizDesk application without ever knowing they are talking to SDP behind the scenes
...
The signup flow begins on the ServizDesk.com marketing website. Customers learn about ServizDesk products, compare plans, and make their purchase decision there. When they are ready to sign up they click a signup or "Get Started" link on the marketing site which hands them off to the SDP-hosted signup flow to complete registration.

The ServizDesk.com website itself is outside the scope of this specification. This section covers the SDP-hosted signup flow that begins after the customer clicks through from the marketing site.

### Signup Steps

1. **Plan Selection**
   - Customer selects ServizDesk Lite (or other available plan)
...
4. **Payment (conditional — paid plans only)**
   - **Paid signup:** Customer is redirected to Stripe Checkout to enter card details.
   ...
   - **Trial signup (no credit card required):** Step 4 is skipped entirely. No Stripe Customer or Subscription is created during signup. The account is provisioned immediately after Step 3 (Password Setup). Payment is collected later if and when the customer converts — see Section 6.3 Trial Conversion below and **ServizDesk Pricing & Billing Specification V2, Section 6** for full trial terms.
...
6. **Account Provisioning**
   ...
   - On provisioning failure the customer is notified and ServizDesk support is alerted
...
> **Decision:** Stripe Billing + Stripe Checkout is used for all payment collection on paid signups. Card data never touches SDP servers at any point. Trial signups do not collect payment — no credit card is required. See ServizDesk Pricing & Billing Specification V2, Section 6.1.

---

### 3.2.1 Trial Signup Path

Trial signups follow the same Steps 1–3 as paid signups (Plan Selection, Business Information, Password Setup) but skip Step 4 (Payment) entirely. No credit card is required.

**Trial-specific behavior:**

- **Plan Selection (Step 1):** Customer selects the Lite plan with the "Start Free Trial" option. The billing cycle selector (monthly/annual) is not shown — it is presented at conversion.
- **Steps 2–3:** Identical to paid signup.
- **Step 4 (Payment):** Skipped. No Stripe Customer or Subscription is created.
- **Step 5 (Security Verification Setup):** Identical to paid signup — required and non-skippable.
- **Step 6 (Account Provisioning):** SDP provisions the SDTA workspace. The tenant record is created with `trial_start_date` set to the current date and `trial_end_date` set to 14 days from creation. No Stripe IDs are stored until the customer converts.
- **Step 7 (Confirmation):** Customer receives a welcome email noting their 14-day trial and is redirected to the SDTA login page.

**Trial lifecycle** (managed by SDTA background task `manage_trial_lifecycle`):

- Days 1–14: Full Lite access with countdown timer.
- Day 15: Account enters Read-Only mode. Persistent banner directs customer to the Billing section to add payment.
- Day 45: Account flagged for cleanup. 60-day data retention grace period begins.
- Day 105: Permanent deletion.

See **ServizDesk Pricing & Billing Specification V2, Section 6** for complete trial terms, expiration flow, and conversion rules.

### 3.2.2 Trial Conversion

When a trial customer adds payment (from the Billing section in SDTA or the conversion prompt):

1. Customer selects their plan (Lite, Plus, or Pro) and billing frequency (monthly or annual).
2. Customer is redirected to Stripe Checkout to enter card details.
3. SDP creates the Stripe Customer and Subscription. Billing starts immediately from the conversion date — no retroactive charge for the trial period.
4. If the account is in Read-Only mode, full access is restored immediately upon successful payment.
5. SDP stores the Stripe Customer ID, Subscription ID, and card metadata (last 4, brand, expiry) per the standard paid account model.

> **Decision:** Trial conversion follows the same Stripe Checkout flow as paid signup Step 4. The only difference is timing — payment is collected at conversion rather than at signup.
...
**Plan & Subscription:**
- View current plan (ServizDesk Lite / Plus / Pro)
...
3. On successful verification, SDP attempts to unlock the SDTA Administrator account. This follows the same 3-try rules as provisioning. If the 3rd attempt fails (e.g., SDTA is momentarily down), the user receives a gentle warning that something internal has gone down and to try again in a few minutes.
...
**Paid signups** follow a strict **verify first, bill second** model. Payment is authorized during signup but is not captured until all provisioning steps have been validated and are ready to commit. If anything fails before that point the payment authorization is cancelled and the customer is never charged. This guarantees that no customer is ever billed for a failed or incomplete account.

**Trial signups** skip all payment phases. No Stripe authorization, capture, or charge occurs. Provisioning proceeds directly from validation to workspace creation.
...
2. **Collect Payment Authorization (Stripe) — paid signups only**
   ...
   - **Trial signups:** This step is skipped entirely. No Stripe interaction occurs during trial provisioning.

3. **Generate Tenant UUID**
   - SDP generates a UUID that will uniquely identify this tenant across all ServizDesk systems
...
1. **Capture Payment — paid signups only**
   ...
   - **Trial signups:** This step is skipped. Phase 2 proceeds directly to tenant record creation.

2. **Create Tenant Record in SDP**
   - SDP creates the authoritative tenant account record.
   - **Paid signups:** Status is marked as `PROVISIONING_IN_PROGRESS`.
   - **Trial signups:** Status is marked as `PROVISIONING_IN_PROGRESS`. The record includes `trial_start_date` and `trial_end_date` (14 days from creation). Stripe Customer ID and Subscription ID are null.
...
3. **Create Workspace in SDTA (The 3-Retry Flow)**
   ...
   - **Retry Logic:** If SDTA fails to respond successfully, SDP waits 5 seconds and retries. If it fails a second time, it waits 15 seconds and retries for a third and final time.

4. **On Success:**
   - Account status in SDP is updated to `ACTIVE`.
   - The Welcome Email is sent to the customer with their login URL.
   - ServizDesk staff are notified of a successful signup.

5. **On Total Failure (Split Brain scenario):**
   - If SDTA completely fails after the 3rd attempt, **the Stripe payment remains captured and the SDP record remains intact.**
...
- For paid signups, the payment authorization is cancelled — the customer is never charged
- For trial signups, no payment was involved — the failure simply stops the process
- No records are created in SDP or SDTA
- The customer is shown a clear error message on the signup page
- If the failure is a card authorization issue (paid signups only), the customer is prompted to try a different card
- If the failure is a system issue, the customer is advised to try again or contact support
- ServizDesk staff are alerted if the failure is due to a system or provisioning validation issue

### Phase 2 Failure (After Payment Capture / After Validation for Trials)
If Phase 2 SDTA provisioning completely fails after the 3 automated attempts:
- **Paid signups:** The Phase 2 transaction is **not** rolled back. SDP retains the captured payment and the master tenant record.
- **Trial signups:** SDP retains the master tenant record. No payment is involved.
- The account status is set to `PROVISIONING_FAILED`.
- A **Critical System Alert** is raised on the SDP dashboard.
- Staff must investigate the raw SDTA logs linked to the alert, fix the infrastructure issue, and press "Retry Provisioning" from the SDP dashboard. Once successful, the account becomes `ACTIVE` (or `ACTIVE` with trial dates for trial signups) and the welcome email is sent.

> **Note:** We prioritize retaining the customer. A human-in-the-loop fix is preferred over a hard rollback that forces the customer to sign up again. For trial signups, no refund consideration is needed since no payment was collected.

---

## 4.4 Staff-Initiated Provisioning

In exceptional circumstances ServizDesk staff may need to manually trigger or re-trigger provisioning for a customer from the SDP back office. This is a staff-only action and requires Administrator-level staff access.
...
1. For paid signups, payment is authorized during signup but captured only after all Phase 1 validation steps pass successfully.
2. The customer is never charged if any Phase 1 step fails.
3. Trial signups skip all payment authorization and capture steps. No Stripe interaction occurs until the customer converts.
4. All Phase 2 database operations execute as a single atomic transaction — partial provisioning is not permitted.
5. If Phase 2 (SDTA provisioning) fails after all automated retry attempts: for paid signups, the captured payment is **retained** and the account is flagged as `PROVISIONING_FAILED` — a Stripe refund is **not** automatically initiated, staff must investigate and manually retry provisioning from the SDP back office, and if staff determine a refund is warranted it can be issued manually (see Section 8.9); for trial signups, no payment is involved so only the tenant record and staff alert are relevant. See Section 4.3 for the full Phase 2 failure model and rationale.
6. Tenant UUIDs are never reused, even after cancellation or failed provisioning.
7. The welcome email and staff notification are sent only after Phase 2 commits successfully.
8. ServizDesk staff are alerted on every provisioning failure and must investigate and resolve manually.
9. Staff-initiated provisioning follows the same two-phase sequence as self-service provisioning.
...
ServizDesk offers the following ServizDesk plans. All plans are managed through SDP.

| Plan              | Status         | Application |
|-------------------|----------------|-------------|
| ServizDesk Lite      | In development | SDTA          |
| ServizDesk Plus      | Planned        | SDTA          |
| ServizDesk Pro       | Planned        | SDTA          |
| ServizDesk Enterprise| Future         | TBD         |
...
Plan limits (seats, storage, features) are defined in **ServizDesk Product Tier Map V2**.

Pricing for all tiers is defined in **ServizDesk Pricing & Billing Specification V2**.

For complete plan limits, see:
→ **ServizDesk Product Tier Map V2, Section 6**

For complete pricing details, see:
→ **ServizDesk Pricing & Billing Specification V2, Section 3**

---

## 5.3 Billing Cycles

Customers choose between monthly and annual billing at signup. Billing cycle rules, pricing, and switching behavior are defined in **ServizDesk Pricing & Billing Specification V2, Section 3.2**.
...
→ **ServizDesk Pricing & Billing Specification V2, Section 3.2**
...
> *"Your account cannot be downgraded to ServizDesk Lite at this time. The following must be resolved first:"*
> - You have 12 active seats. ServizDesk Lite allows a maximum of 10.

---

## 5.6 Cancellations

A customer may cancel their account from the Billing section in SDTA or by contacting ServizDesk support.
...
The post-cancellation timeline has two distinct phases before permanent deletion:

**Phase 1 — Cancelled (Pending Expiry)** — from cancellation confirmation until the billing period end date:
...
**Phase 2 — Cancelled (Read Only)** — 30 days immediately following the billing period end date:
- On the billing period end date, the account transitions to **Cancelled (Read Only)** status
- All users can log in and view all data but cannot create, edit, or delete records
- The customer can still export their data via CSV during this window
- This 30-day window is intended to give customers a final opportunity to retrieve their data before access is fully revoked

### Data Retention After Cancellation
- After the 30-day Cancelled (Read Only) window, the account transitions to **Cancelled** status and all SDTA access is revoked
- All tenant data in SDTA is retained for a further **60 days** from the access revocation date
- During the 60-day retention period the customer may reactivate their account and regain full access to all their data (see Section 5.7)
- After the 60-day retention period all tenant data in SDTA is permanently deleted via an **asynchronous background worker** during off-hours, ensuring the heavy database operation does not impact live application performance
- This deletion is permanent and cannot be recovered
- A data deletion warning email is sent to the billing email address **7 days** before permanent deletion
- Total timeline from billing period end date to permanent deletion: **90 days** (30-day read-only + 60-day data retention)

---

## 5.7 Reactivation

A customer whose account is in **Cancelled (Read Only)** or **Cancelled** status may reactivate at any time before permanent deletion. Reactivation is not possible once the 60-day data retention period has elapsed.

### Reactivation Behavior
- Customer contacts ServizDesk support or uses a reactivation link in the data deletion warning email
...
After the 60-day data retention period has elapsed reactivation is not possible. The customer would need to sign up as a new account.
...
4. Cancellations take effect at the end of the current billing period — full read access continues until then, but no new records may be created once cancellation is confirmed.
5. No new records can be created in SDTA after a cancellation is confirmed.
6. Following the billing period end date, tenants enter a 30-day **Cancelled (Read Only)** window, then a 60-day data retention window — 90 days total before permanent deletion.
7. Customers may reactivate and regain full data access at any time during the Cancelled (Read Only) or Cancelled (data retention) windows, up to 90 days after the billing period end date.
8. After the 60-day data retention period all tenant data is permanently deleted and cannot be recovered.
9. A data deletion warning email is sent **7 days** before permanent deletion.
...
All billing for ServizDesk products is managed through SDP using ServizDesk's own Stripe account. ServizDesk is the merchant of record for all customer charges. Customers are billed by ServizDesk — not by the ServizDesk applications themselves.
...
## 6.2 ServizDesk Stripe Account

ServizDesk maintains a single Stripe account used to bill all ServizDesk customers. This is entirely separate from the Stripe accounts that tenants use within SDTA to bill their own customers.

- ServizDesk is the merchant of record for all ServizDesk subscription charges
- All subscription billing flows through the ServizDesk Stripe account
...
- ServizDesk never stores raw card data — see Section 6.8 for full PCI posture

---

## 6.3 Billing Model

ServizDesk uses per-seat subscription billing with both monthly and annual options. Complete billing model details, pricing, and seat counting rules are defined in **ServizDesk Pricing & Billing Specification V2, Section 3**.
...
- Billing anniversary date is set at initial signup for paid accounts, or at conversion date for trial-to-paid conversions
...
Seat billing rules and seat counting logic are defined in **ServizDesk Pricing & Billing Specification V2, Section 3.3**.
...
→ **ServizDesk Pricing & Billing Specification V2, Section 3.3**

---

## 6.5 Storage Add-On Billing

Storage add-on pricing and limits are defined in **ServizDesk Pricing & Billing Specification V2, Section 4**.
...
→ **ServizDesk Pricing & Billing Specification V2, Section 4**
...
### ServizDesk Staff Notification
- SDP flags the account as suspended in the back office
- ServizDesk staff are notified of the suspension via the SDP staff notification system
...
Handling payment card data is subject to PCI DSS (Payment Card Industry Data Security Standard) regulations. ServizDesk's architecture is specifically designed to minimize PCI scope and risk by ensuring that raw card data never touches ServizDesk systems at any point.

### How Card Data Is Handled
- All card data entry is handled exclusively through **Stripe Checkout** — a Stripe-hosted payment page
- When a customer enters their card details they are entering them directly into Stripe's systems, not ServizDesk's
...
- SDP never renders a custom card entry form that posts card data to ServizDesk servers
- No ServizDesk staff member ever has access to a customer's full card number

> **Decision:** ServizDesk's use of Stripe Checkout for all card collection dramatically reduces PCI DSS scope. ServizDesk operates as a SAQ A merchant — the lowest PCI compliance tier — because card data never touches ServizDesk servers or staff at any point.

---

## 6.9 Governance Rules

1. All ServizDesk subscription billing flows through the ServizDesk Stripe account — ServizDesk is the merchant of record.
...
10. Stripe generates and sends payment receipts to customers automatically — ServizDesk does not send separate billing receipts.
...
These credentials exist to answer one question: when an Administrator is locked out of SDTA and cannot log in, how does ServizDesk verify that the person requesting recovery is the legitimate account owner?
...
- No ServizDesk staff member can view the plaintext value of any security answer or PIN
...
> **Note:** This list is managed in SDP and is shared across all ServizDesk plans. It may be expanded in future versions.

---

## 7.4 Credential Verification Process

Security credentials are verified by ServizDesk support staff during the Administrator account recovery process (see Section 8.3 for the full recovery workflow).
...
> **Decision:** PIN verification is attempted first. Security questions are the primary second factor. Billing details are the last resort fallback only. All three methods require ServizDesk staff involvement — there is no fully automated self-service recovery path.
...
3. No ServizDesk staff member can view the plaintext value of any security credential.
4. Administrator recovery requires ServizDesk staff involvement — there is no fully automated self-service recovery path.
...
# 8. ServizDesk Staff Tools (Back Office)

---

## 8.1 Overview

The ServizDesk Staff Back Office is the internal face of SDP. It is used exclusively by ServizDesk staff to manage customer accounts, resolve support issues, and perform operational tasks that cannot be done through the customer-facing surface.

All ServizDesk staff have full access to all back office functions. There are no role-based restrictions within the staff back office in the initial version — access control is covered in Section 10.
...
| Plan type                 | Filter by ServizDesk Lite / Plus / Pro        |
...
When an Administrator is locked out of SDTA they contact ServizDesk support. Staff handle the recovery from the back office.
...
### Process
1. Staff locates the customer account
2. Staff adjusts the seat count within the plan limit (maximum 10 for ServizDesk Lite)
...
4. The standard cancellation behavior applies (see Section 5.6):
   ...
   - 30-day Cancelled (Read Only) window begins at billing period end, followed by 60-day data retention — 90 days total before permanent deletion
...
To ensure proactive resolution of system failures (such as a `PROVISIONING_FAILED` event or internal API sync failure), SDP includes a robust Critical System Alerts dashboard. This acts as a lightweight, built-in incident tracking system for ServizDesk staff.
...
1. All ServizDesk staff have full access to all back office functions.
...
SDP is responsible for all transactional communications between ServizDesk and its customers. These are operational emails triggered by account and billing events — not marketing emails.

All emails are sent from SDP via **Postmark** (see Section 9.4). All emails use basic HTML templates with ServizDesk branding — company name, logo, and consistent styling. Full design templates are deferred to a future version.
...
- **Content:**
  ...
  - SDTA login URL
  - Link to getting started guide (ServizDesk.com)
  - ServizDesk support contact
...
- **Content:**
  ...
  - Reason for suspension (payment failure or ServizDesk-initiated)
...
- **Content:**
  ...
  - Statement that read-only access continues for 30 days after the billing period end, then data is retained for a further 60 days (90 days total)
  - Instructions for exporting data via CSV before access is fully revoked
  - Reactivation information — account can be reactivated at any time within the 90-day window
  - ServizDesk support contact

---

### E06 — Data Deletion Warning
- **Trigger:** 7 days before permanent data deletion (60-day data retention period minus 7 days after SDTA access is revoked)
- **Recipient:** Billing email address
- **Purpose:** Final warning before all tenant data is permanently deleted
- **Content:**
  - Warning that all account data will be permanently deleted in 7 days
  - Exact deletion date
...
- **Trigger:** Successful account reactivation within the 90-day window (Cancelled (Read Only) or Cancelled status)
- **Recipient:** Billing email address
...
- **Note:** ServizDesk should configure Stripe to send automatic receipt emails to customers. No SDP development is required for this email type.
...
3. The data deletion warning (E06) is scheduled 53 days after the access end date (7 days before the 60-day deletion deadline)
4. If an email delivery fails SDP logs the failure and alerts ServizDesk staff
...
> **Decision:** Postmark is the designated email provider for **all ServizDesk email sending** — both internal SDP communications (Welcome emails, billing alerts, password resets) and tenant-to-customer emails (quotes, invoices, notifications).
> **Tenant-Side Emails:** Tenant-to-customer email sending uses a point-based system (1 point = 1 email). Platform-managed mode sends from ServizDesk's domain by default. Custom Domain authentication (emails from tenant's own domain) is a paid add-on available on Pro and Enterprise. BYOS (Bring Your Own SMTP) is not supported. See ServizDesk Email Specification V1 and Pricing & Billing Specification V2 Section 10A and 11.4 for full details.
...
3. All emails use basic HTML templates with ServizDesk branding.
4. Email delivery failures are logged and ServizDesk staff are alerted.
5. The data deletion warning email is sent 7 days before permanent deletion.
6. SDP does not send marketing emails — transactional operational emails only.

---

# 10. ServizDesk Staff Roles & Access

---

## 10.1 Purpose

This section defines how ServizDesk staff authenticate into the SDP back office and how their accounts are managed. All ServizDesk staff have full access to all back office functions — there are no role-based restrictions within the staff back office at this stage.

---

## 10.2 Staff Authentication

ServizDesk staff authenticate into the SDP back office using a username and password.

- **Username:** ServizDesk staff email address
...
Staff accounts are created and managed manually by the SDP Administrator — the designated ServizDesk staff member responsible for managing the SDP back office itself.
...
- When a staff member leaves ServizDesk the SDP Administrator deactivates their account immediately
...
The SDP Administrator is the designated ServizDesk staff member responsible for managing the SDP back office itself. This is an internal designation — not a separate system role.
...
There must always be at least one designated SDP Administrator at ServizDesk. If the SDP Administrator leaves the company a replacement must be designated before the departing Administrator's account is deactivated.

---

## 10.5 Access Control

All ServizDesk staff have full access to all SDP back office functions. There are no role-based restrictions in the initial version.

> **Note:** Role-based access control for ServizDesk staff may be introduced in a future version as the team grows and operational needs require it. The architecture should not preclude this being added later.

---

## 10.6 Governance Rules

1. All ServizDesk staff authenticate using a username and password — no self-registration is permitted.
...
8. There must always be at least one designated SDP Administrator at ServizDesk.
...
This section defines how SDP handles data retention, credential security, and ServizDesk's overall security posture. Many of the rules in this section are a direct consequence of decisions made in earlier sections — they are consolidated here as the authoritative reference.
...
- Stripe transaction records are retained by Stripe indefinitely and are accessible via the ServizDesk Stripe dashboard

### Cancelled Accounts
| Phase                         | Duration                                       | Data State                              |
|-------------------------------|------------------------------------------------|-----------------------------------------|
| Active                        | Until billing period end date                  | Full SDTA access; no new records after cancellation confirmed |
| Cancelled (Pending Expiry)    | From cancellation until billing period end     | Full read access, no new record creation; CSV export available |
| Cancelled (Read Only)         | 30 days after billing period end               | All users may log in and view data; no create, edit, or delete |
| Cancelled                     | Days 31–90 after billing period end (60 days)  | SDTA access fully revoked; data preserved in SDTA database |
| Permanently Deleted           | After 90-day total window                      | All tenant data in SDTA permanently deleted (via background worker) |
...
- A data deletion warning email is sent 7 days before permanent deletion (see Section 9 — E06)
...
- This is governed by the ServizDesk Lite specification (Section 18)
...
- No ServizDesk staff member can view a customer's plaintext password

### SDTA Employee Passwords
- Minimum 8 characters
- Must include uppercase, lowercase, number, and special character
- Never stored in plaintext — hashed using bcrypt or equivalent
- No ServizDesk staff member can view an employee's plaintext password

### ServizDesk Staff Passwords
...
- No ServizDesk staff member can view the plaintext value of any security credential
...
ServizDesk's architecture is specifically designed to minimize PCI DSS scope by ensuring raw card data never touches ServizDesk systems at any point.
...
### PCI Compliance Tier
ServizDesk operates as a **SAQ A merchant** — the lowest and simplest PCI compliance tier. This is achievable because:
- Card data never touches ServizDesk servers
- No ServizDesk staff member ever has access to a customer's full card number
- All card collection is delegated entirely to Stripe

> **Important:** ServizDesk must never introduce a custom card entry form that posts card data to SDP servers. Doing so would immediately invalidate SAQ A status and dramatically increase PCI compliance obligations. All card collection must always go through Stripe Checkout.
...
5. ServizDesk must always use Stripe Checkout for card collection — custom card entry forms are strictly prohibited.
6. Tenant data in SDTA is retained for a total of 90 days after billing period end (30-day Cancelled (Read Only) + 60-day Cancelled data retention), then permanently deleted.
...
10. No ServizDesk staff member can view the plaintext value of any password or security credential.
...
1. The paid signup flow is accessible from ServizDesk.com via a signup link and completes successfully end to end — plan selection, business information, password setup, Stripe Checkout payment, security verification setup, provisioning, and welcome email.
1a. The trial signup flow completes successfully end to end — plan selection (Lite with "Start Free Trial"), business information, password setup, security verification setup, provisioning, and welcome email — with no credit card required. Step 4 (Payment) is skipped entirely for trials.
2. Payment authorization and capture follow the verify first, bill second model for paid signups — no customer is charged for a failed provisioning. Trial signups involve no payment interaction.
...
8. Monthly and annual billing subscriptions are created correctly in Stripe on paid signup. Trial accounts have no Stripe subscription until conversion.
...
11. Cancellation stops future billing, access continues to end of billing period, the 30-day Cancelled (Read Only) window begins at billing period end, and the 60-day data retention countdown begins correctly after that (90 days total).
...
27. The data deletion warning email is scheduled and sent correctly 7 days before permanent deletion.
...
29. Tenant data in SDTA is permanently deleted after the full 90-day window expires (30-day Cancelled (Read Only) + 60-day Cancelled data retention).
30. SDP audit log and billing records are retained for 36 months on a rolling basis.
31. Account reactivation within the 90-day window fully restores SDTA access and data.
...
**End of ServizDesk Platform (SDP) V2 Specification**