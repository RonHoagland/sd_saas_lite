# ServizmaDesk System Status Specification V3
...
| **Terminated** | Employment has ended. | **Does not count against purchased seats.** Seat becomes vacant. | Requires a `termination_date`. System access immediately revoked. Record is preserved for history. A new employee may be invited into the vacated seat without triggering an additional Stripe charge. Reversion to Active requires Admin and a `reason`. |
...
## 22. Communication Template Lifecycle
**Transitions:** `Draft` → `Active` → `Inactive`
...
*   **Inactive**: Template retired from active use. Existing `CommTriggerTemplates` references are preserved and continue to function for historical log purposes. Cannot be assigned to new Triggers.

---

## 23. Communication Trigger Lifecycle
**Transitions:** `Draft` → `Active` → `Inactive`
...
> [!NOTE]
> A Trigger must have at least one Active Template assigned via `CommTriggerTemplates` before it can be set to Active. Attempting to activate a Trigger with no assigned Active Templates returns a validation error.

---

## 24. Pricebook Entry Lifecycle
...
## 25. WorkGroup Division (Epic) Lifecycle
**Transitions:** `Open` → `In Progress` → `Completed` / `Cancelled`
...
| V2 | March 2026 | Expanded lifecycle coverage across all core entities. Added Fleet Management (Sections 17.1–17.2). |
| V3 | March 2026 | Added Employee Lifecycle (Section 19) including seat billing rules and Lite tier cap enforcement. Added Inventory Lifecycle (Section 20) with Hold status and system-managed stock flags (`is_low_stock`, `is_out_of_stock`). Added Vendor Lifecycle (Section 21) with Do Not Use status. Added Communication Template Lifecycle (Section 22). Added Communication Trigger Lifecycle (Section 23). Added Pricebook Entry Lifecycle (Section 24). Added WorkGroup Division Lifecycle (Section 25). |
