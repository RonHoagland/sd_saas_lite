# ServizmaDesk
# ServizmaDesk Lite – MVP V3 Specification (Working Draft)
...
Lite tier pricing is defined in **ServizmaDesk Pricing & Billing Specification V1, Section 3.1**.

**Current Lite Pricing:**
- $35 per seat per month (monthly billing)
- $29 per seat per month (annual billing)
...
Storage upgrade pricing is defined in **ServizmaDesk Pricing & Billing Specification V1, Section 4**.
...
For SDP provisioning and billing details, see:
→ **ServizmaDesk Platform (SDP) Specification V1**
...
Password resets for non-Administrator employees are performed by an Administrator from the Admin Area.

Process:
1. Administrator navigates to the Employee record in the Admin Area
2. Administrator selects Reset Password
3. System sends a password reset link to the employee's company email address
4. Employee clicks the reset link and sets their new password
5. Reset link expires after 24 hours if not used
6. Employee is required to log in with their new password after reset

> **Note:** There is no self-service password reset link for standard employees in Lite. All resets are initiated by the Administrator.
...
- A Asset with any Work Order history cannot be deleted. The Work Orders must be deleted first.
...
2. **Company Profile** — prompts the Administrator to complete their company name, address, phone, and logo (pulls from SDP signup data as defaults)
...
- Create/Edit
- Assign to user
- Link to Company / Asset / Quote / Work Order
- Status tracking
- CSV export
...
- `object_type` (e.g., Company, Asset, WorkOrder, Invoice, Product)
...
- Company, Asset, Quote, Work Order, Invoice, Product, Task
...
1. End-to-end workflow functions:
   Company → Asset → Quote → Work Order → Invoice → Payment
