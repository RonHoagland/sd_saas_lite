...
**Document Version:** V1
**Status:** Approved (Resolves Gap 2.9)
...
- **Triggers**: Whenever a write operation occurs on `TenantPreference`, `Customer`, `Asset`, `StripeConnection`, or `User`, a background signal (e.g., Django `post_save`) MUST evaluate the corresponding logic and update the `checklist_items` JSONField in the `OnboardingState` record.
- **Visibility**: Once all non-optional items (2.1–2.4) are marked as `True`, the dashboard checklist widget should be automatically hidden for the tenant.
