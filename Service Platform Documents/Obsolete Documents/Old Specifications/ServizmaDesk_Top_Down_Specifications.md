...
- **Request Number** — Auto-generated (ServizmaDesk record numbering: SR26-0001)
- **Customer / Lead** — Link to Customer record (or "Unknown/New Lead" details)
...
- **Work Orders** — When triaged successfully, a Service Request converts into a Work Order or Quote
...
- **Customer** — Link to Customer record (required)
- **Asset** — Link to Asset record (recommended, not required)
  - Multiple Assets can be linked to one Work Order
...
**Requisition Fields:**
- **Requisition Number** — Auto-generated (ServizmaDesk record numbering: RQ26-0001)
...
- **Status** — Dropdown
  - Values: Draft, Submitted, Partially Fulfilled, Fulfilled, Cancelled
...
> **Architectural Decision:** ServizmaDesk actively avoids fragile, two-way API syncs with external accounting platforms (like QuickBooks Online) due to historically high failure rates, API deprecations, and support burdens. Instead, ServizmaDesk utilizes a **"Native Accounting First"** approach that scales with the customer's tier, supported by robust, bulletproof CSV exports for external tax prep.
...
## 1.18 Fleet / Vehicle Entity (Pro/Enterprise)
...
**Purpose:** Define recurring service schedules for customer-owned Assets. This is the primary recurring revenue mechanism for HVAC, plumbing, and electrical businesses.

**Plan Fields:**
- **Plan Name** — Text (e.g., "Annual HVAC Tune-Up", "Quarterly Filter Service")
- **Customer** — Link to Customer record (required)
- **Asset(s)** — Link to Asset(s) covered by this plan (required)
- **Plan Status** — Active, Paused, Expired, Cancelled
- **Start Date** — Date picker
...
- **Checklist Template** — Auto-apply checklist to generated Work Orders
- **Assigned To** — Default technician or assignment pool
- **Auto-Generate Work Orders** — System creates Work Orders per schedule
- **Advance Generation** — How far ahead to create upcoming Work Orders (configurable)

**Plan Pricing (Pro/Enterprise):**
- **Plan Tiers** — Define multiple service levels
  - Example: Silver, Gold, Platinum
  - Different services/inclusions per tier
- **Pricing** — Monthly, quarterly, or annual fee
- **Recurring Invoice Generation** — Auto-generate invoices per billing cycle
- **Discounts** — Plan members receive percentage discount on additional work

**Maintenance Plan Relationships:**
- Linked to Customer (required)
- Linked to Asset(s) (required — asset-centric)
- Generates Work Orders per schedule
- Generates Invoices per billing cycle (Pro/Enterprise)
...
**Platform Approach:**
- ServizmaDesk uses a **responsive web application** accessible from any modern mobile browser
...
> **Design Principle:** By delivering a single responsive web codebase, ServizmaDesk avoids the mobile parity gaps that plague competitors (FieldPulse: features missing on mobile, HousecallPro: Android 3.3/5 rating, Workiz: Android 3.0/5 rating). Every feature works on every device from Day 1.
...
- **Request Service (Call Intake)** — Customers submit service or callback requests via web widget or portal
  - Request appears directly in ServizmaDesk as a **Service Request** (Section 1.3)
...
- Appears in ServizmaDesk as an inbound **Service Request** (Section 1.3)
...
> **Architectural Boundary:** ServizmaDesk does **not** support live, two-way API syncs with QuickBooks or Xero. Instead, ServizmaDesk relies on a highly stable **Standardized Accounting Export** format.
...
- **Purpose:** One-way read-only sync from the user's bank into ServizmaDesk Enterprise's native General Ledger for Bank Reconciliation. No data is pushed *out* to external systems.
...
- **No markup** — ServizmaDesk does not take a cut of payment processing
- **Webhook processing** — Automated payment recording
- **PCI SAQ A** — Card data never touches ServizmaDesk servers
...
> **Architectural Boundary:** ServizmaDesk treats Payroll as an explicitly offloaded function. The system tracks Employee Time (clock-in/out, job hours) and Commissions, but wage calculation, tax withholding, and direct deposits are pushed to specialized third-party payroll providers.
...
**Communication:**
- **Email Provider** — Configurable SMTP or Point-based system for tenant emails
...
- **REST API** — Programmatic access to ServizmaDesk data (Pro/Enterprise)

---

# 8. Custom Workflows & Forms (Pro/Enterprise)

## 8.1 Custom Status Workflows

### Workflow Configuration

**Purpose:** Configure Work Order workflows specific to business type or service category

**Custom Workflow Features:**
- **Create custom Work Order statuses** beyond default Draft/Scheduled/In Progress/On Hold/Completed/Closed
- **Example workflows:**
  - **Plumbing:** "Site Assessment" → "Shutoff and Drain" → "Remove Old" → "Install New" → "Pressure Test" → "Restore" → "Customer Sign-Off"
  - **Electrical:** "Permit Applied" → "Material Ordered" → "Installation" → "Inspection Scheduled" → "Inspection Passed" → "Customer Approval"
  - **HVAC:** "Diagnostic" → "Quote Approval" → "Equipment Ordered" → "Installation" → "System Test" → "Complete"

**Workflow Enforcement:**
- **Required steps** — Block Work Order closure until mandatory steps are complete
- **Guided progression** — Technicians follow workflow in sequence
...
> **Design Note:** These settings define the default prefix and starting number for auto-generated record numbers. If a user leaves fields blank during setup, the system applies these defaults. The format follows the ServizmaDesk convention: Prefix + Number (e.g., C26-0001).
...
**Project Numbering:**
- **Project Prefix** — Text (default: PJ)
- **Project Start Number** — Number (default: 0001)
...
**Product Numbering:**
- **Product Prefix** — Text (default: XT)
...
## 10.6 Email SMTP Configuration

### Outgoing Email Setup

> **Scope:** These settings allow the tenant to configure their own SMTP server for sending out emails (Quotes, Invoices, Work Order updates, etc.) to customers.

**SMTP Credentials:**
- **SMTP Host** — Text (e.g., smtp.gmail.com, smtp.office365.com)
- **SMTP Port** — Number (typically 587, 465, or 25)
- **SMTP Username** — Text
- **SMTP Password** — Password (encrypted/masked)
- **Use TLS** — Toggle (yes/no)
- **Use SSL** — Toggle (yes/no)

**Sender Information:**
- **From Name** — Text (the name that appears in the customer's inbox, e.g., "Acme HVAC Services")
- **From Email Address** — Email (the email address the customer replies to, e.g., service@acmehvac.com)
...
2. **Create multiple Work Orders** → Each phase = separate Work Order under project
3. **Assign employees** → Different employees per phase
4. **Track progress** → Monitor completion by Work Order status
5. **Track costs** → Labor hours + materials vs. budget
6. **Invoice per phase** → Or invoice at project completion
7. **Close project** → Mark complete when all Work Orders done
...
Customer (1) ←→ (many) Maintenance Plans
...
Asset (1) ←→ (many) Maintenance Plans
Asset (1) ←→ (many) Documents/Photos
Asset (0..1) ←→ (many) Child Assets [parent/child - Pro/Enterprise]

Work Order (many) ←→ (1) Customer [required]
Work Order (many) ←→ (many) Assets [recommended, asset-centric link]
Work Order (many) ←→ (1) Project [optional]
...
Maintenance Plan (many) ←→ (1) Customer [required]
Maintenance Plan (many) ←→ (many) Assets [required - asset-centric]
Maintenance Plan (1) ←→ (many) Work Orders [auto-generated]
...
- Add-on modules (Fleet Management) use the same shared-schema, tenant-scoped pattern as all other entities
...
- The Vehicle optional field on Work Orders is present in the schema from day one but hidden in the UI when Fleet Management is not active
...
# 13. Fleet Management (Add-On Module)

## 13.1 Overview

Fleet Management is a **paid add-on module** available exclusively to Pro and Enterprise tier tenants. It is not included in any base tier price. The module activates when the tenant's add-on subscription is active. When inactive, all Fleet Management UI is hidden and all Fleet Management controllers reject write operations.

**Availability:** Pro (add-on), Enterprise (add-on)
**Pricing:** Flat per-tenant per-month rate (not per-seat, not per-vehicle). Price to be defined in the ServizmaDesk Pricing & Billing Specification V2, Section 11.3.

**Design rationale:** Fleet vehicles are operationally distinct from customer-owned equipment (Assets). A service van has a completely different lifecycle, ownership structure, and management needs than the HVAC unit a technician is servicing. Fleet Management is therefore a separate first-class module with its own Vehicle entity, not an extension of the Asset entity.
...
**Core Vehicle Fields:**
- **Vehicle Number** — Auto-generated (ServizmaDesk record numbering: V26-0001)
- **Status** — Dropdown
...
- Work Orders gain an optional **Vehicle** field when Fleet Management is active (the vehicle dispatched for the job)
- This field is hidden on Work Orders when the Fleet Management add-on is not active
- Vehicle is not required on a Work Order — many jobs are walk-in or the vehicle is not tracked

---

## 13.6 Fleet Management UI Areas
...
Configurable per tenant in the Fleet Management preferences area:
...
> **Full technical architecture details are maintained in the ServizmaDesk Technical Architecture V1.** This section provides a summary reference for context within this document. All technical decisions and version specifications are owned by the Technical Architecture document.
...
3. **Many-to-Many Relationships:** A Work Order can link to multiple Assets. An Invoice can link to multiple Work Orders (progress/batch billing).
4. **Parent/Child Architecture:** Asset table includes nullable `parent_id` foreign key to support nested equipment structures for commercial clients.
...
- **60-day data deletion worker** — permanently deletes tenant data after grace period expires
...
### Email
- **Platform Email (SDP → Tenant):** Transactional emails from ServizmaDesk to tenant account holders (welcome, password reset, provisioning alerts) are sent via **Postmark**.
- **Tenant Email (SDTA → Customer) — Plus and above only:**
  - **Mode 1 (ServizmaDesk-managed):** Sent through ServizmaDesk infrastructure. Daily send limits apply. No configuration required from tenant.
  - **Mode 2 (Bring Your Own SMTP):** Tenant configures their own email provider credentials in Preferences. No daily limit. Sent through tenant's own email infrastructure.
  - Tenants select their preferred mode in Preferences.
- **Lite:** No system email sending. Users send from their own email client externally.

### SMS
- **Available in:** Plus and Pro tiers only.
- **Model:** Point-based. 1 point = 1 outbound SMS message.
- **Included allotments and add-on pricing:** See ServizmaDesk Pricing & Billing Specification V2, Section 10.
- **Provider selection:** Deferred to Plus tier specification (Twilio or equivalent).

## 14.6 Payment Architecture

- **Platform Billing (ServizmaDesk subscription charges):** Stripe Checkout. ServizmaDesk never stores raw card data.
...
- **PCI DSS Posture:** ServizmaDesk qualifies for PCI SAQ A. All card data handling delegated to Stripe.
- Full details in ServizmaDesk Pricing & Billing Specification V2, Section 9.
...
| Technical stack, infrastructure, tenancy architecture, security | **Technical Architecture V1** |
| Complete functional scope, entity definitions, data model, system workflows | **Top-Down Specifications V1** (this document) |
| Lite-specific module detail, seat counting rules, UI specifications | **Lite MVP V4 Specification** |
| Platform provisioning, SDP operations, tenant lifecycle | **Platform (SDP) Specification V2** |

## 15.2 This Document Depends On

- **ServizmaDesk Pricing & Billing Specification V2** — for all pricing, trial, and Founding Partner details
- **ServizmaDesk Product Tier Map V2** — for feature gate definitions per tier
- **ServizmaDesk Technical Architecture V1** — for stack, infrastructure, and tenancy decisions

## 15.3 Documents That Reference This Document

- **ServizmaDesk Product Tier Map V2** — references entity definitions and architectural principles
- **ServizmaDesk Lite MVP V4 Specification** — references entity definitions for Lite-scope modules
- **ServizmaDesk Plus Specification (future)** — will reference entity definitions for Plus-scope modules
- **ServizmaDesk Pro Specification (future)** — will reference entity definitions for Pro-scope modules
...
> **Note:** This is a high-level summary. Detailed tier specifications are maintained in separate documents (Lite MVP V4 Spec, Plus Spec, Pro Spec, Enterprise Spec). **All pricing is maintained exclusively in the ServizmaDesk Pricing & Billing Specification V2 — this table contains no pricing data.**
...
| **Communication** | | | | |
| SMS (point-based) | — | ✓ | ✓ | ✓ |
| System Email (ServizmaDesk-managed) | — | ✓ | ✓ | ✓ |
...
> **Pricing:** See ServizmaDesk Pricing & Billing Specification V2 for all tier pricing, seat limits, storage included, and storage add-on options.

---

**End of ServizmaDesk Top-Down Specifications V1**
