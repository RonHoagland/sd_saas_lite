# ServizmaDesk — Internal REST API Specification V1
...
This document defines the complete Internal REST API contract between the **ServizmaDesk Platform (SDP)** and the **ServizmaDesk Tenant App (SDTA)**. It specifies every endpoint, request payload, response format, authentication mechanism, error codes, and retry behavior.
...
| `subdomain` | string | ✓ | Tenant's SDTA subdomain (e.g., `acmehvac` → `acmehvac.servizmadesk.com`) |

### SDTA Actions (All-or-Nothing Transaction)

SDTA executes the following in a single atomic transaction:

1. Create `TenantState` record (`status=Active`, `tier`, `seat_limit`, `storage_limit_bytes`)
2. Create `TenantPreference` record (seed with `company_name`, system defaults for all other fields)
3. Create Administrator `User` record (`email`, `first_name`, `last_name`, `password_hash`, `role=Administrator`, `status=Active`)
4. Create `StorageTracker` record (`total_bytes_used=0`)
5. Create `OnboardingState` record (`wizard_completed=False`)
6. Create `SequenceTracker` records for all entity types (Customer, Asset, WorkOrder, Quote, Invoice, Payment, Task, Product, Employee). Seed each `last_value` as `[start_number] - 1` using the values defined in the newly created `TenantPreference` record.
7. Create initial `Notification` record: "Welcome to ServizmaDesk! Complete your setup with the onboarding checklist."
...
| `reason` | string | ✓ | Free text — logged in SDTA AuditEvent |
...
| Value | SDTA Behavior |
|---|---|
| `active` | Full access restored for all users |
| `suspended` | All employee logins blocked except Administrator (Admin Area only) |
| `read_only` | All users can log in; no creates/edits/deletes permitted |
| `cancelled_pending_expiry` | No new records; read/export access continues until billing period end |
| `cancelled_expired` | All SDTA access revoked; 60-day deletion countdown begins |
| `pending_deletion` | Celery worker begins permanent hard-delete of all tenant data |
...
**Note:** At least one of `tier`, `seat_limit`, `storage_limit_bytes`, or `add_ons` must be provided.
...
5. Set `status = Active`
6. Log an `AuditEvent`: `action=Unlocked, entity_type=User, entity_id=<user_id>`
...
Called by SDTA whenever an employee is added or terminated, changing the billable seat count. SDP updates the Stripe subscription quantity immediately.
...
|---|---|---|---|
| `tenant_id` | UUID | ✓ | |
| `active_seat_count` | integer | ✓ | Current total billable seat count after the change |
| `employee_id` | UUID | ✓ | The employee that was added or terminated |
| `change_type` | enum | ✓ | `added`, `terminated` |

**Seat counting rules (reminder):** Active + On Leave + Inactive employees count. Terminated employees (with `termination_date` populated) do not count.
...
| Code | Error Code | Meaning |
|---|---|---|
| `422` | `SEAT_LIMIT_EXCEEDED` | `active_seat_count` exceeds the tenant's current `seat_limit`; SDTA must block the employee add before calling |
...
Certain Celery background tasks operate across all tenants simultaneously (e.g., the 60-day deletion worker, the storage reconciliation job, the audit event purge). These tasks cannot set `app.current_tenant_id` to a single tenant's UUID because they need to process multiple tenants' data.
...
|---|---|
| Governed by (inter-app architecture) | ServizmaDesk Technical Architecture V2, Sections 7.1–7.2 |
| Governed by (provisioning sequence) | ServizmaDesk Platform (SDP) Specification V2, Section 4 |
| Governed by (account lifecycle) | ServizmaDesk Platform (SDP) Specification V2, Sections 5–6 |
| Governed by (SDTA data models) | ServizmaDesk Data Models V2 |
| Governed by (database users/RLS) | ServizmaDesk Database Specification V1 |

---

*End of ServizmaDesk Internal REST API Specification V1*
