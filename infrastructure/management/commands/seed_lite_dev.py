...
CUSTOMERS = [
    # (company_name, account_type, status, primary_person_name|None)
    ('Anderson Residence',     'Residential', 'Active', ('John',  'Anderson')),
    ('Rivera Family Home',     'Residential', 'Active', ('Maria', 'Rivera')),
    ('Patel Household',        'Residential', 'Active', ('Raj',   'Patel')),
    ('Greenfield Office Park', 'Commercial',  'Active', None),
    ('Northbridge Cafe',       'Commercial',  'Active', None),
]
...
class Command(BaseCommand):
    ...
    def _seed_customers(self, tenant_id, admin):
        created = []
        for company_name, account_type, status, person_name in CUSTOMERS:
            primary_person = None
            if person_name:
                first, last = person_name
                primary_person = Person.objects.create(
                    tenant_id=tenant_id,
