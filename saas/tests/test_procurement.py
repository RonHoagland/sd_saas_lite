...
from procurement.models import (
    ...
    Requisition, RequisitionLine, Vendor, VendorAccount, VendorBill,
)
...
# ─── VendorAccount ────────────────────────────────────────────────────────────
#
# Mirrors the Customer/Account split: each Vendor gets a 1:1 VendorAccount
# auto-created via crm-style post_save signal, holding billing/credit fields
# that describe how the tenant does business with the vendor.

class VendorAccountTest(SDTATestCase):

    def test_account_auto_created(self):
        v = Vendor.objects.create(name='AutoAcct Vendor')
        self.assertIsNotNone(v.account)
        self.assertEqual(v.account.credit_status, 'Good')
        self.assertEqual(v.account.tenant_id, v.tenant_id)

    def test_account_cascade_delete(self):
        v = Vendor.objects.create(name='Cascade Vendor')
        a_id = v.account.id
        v.delete()
        self.assertFalse(VendorAccount.objects.filter(id=a_id).exists())

    def test_payment_terms_lives_on_account(self):
        v = Vendor.objects.create(name='Terms Vendor')
        v.account.payment_terms = 'Net 45'
        v.account.save()
        v.refresh_from_db()
        self.assertEqual(v.account.payment_terms, 'Net 45')

    def test_tax_id_encrypted_at_rest(self):
        from django.db import connection
        v = Vendor.objects.create(name='Tax Vendor', tax_id='SECRET-EIN-555')
        with connection.cursor() as cur:
            cur.execute(
                'SELECT tax_id FROM procurement_vendor WHERE id = %s',
                [str(v.id)],
            )
            raw = cur.fetchone()[0]
        self.assertTrue(raw.startswith('v1:'))
        self.assertNotIn('SECRET-EIN-555', raw)
        # Roundtrip via ORM still returns plaintext
        v.refresh_from_db()
        self.assertEqual(v.tax_id, 'SECRET-EIN-555')
