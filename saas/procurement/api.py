...
from .models import (
    Vendor, VendorAccount, PurchaseOrder, PurchaseOrderLine, Receiving, LotInfo,
    VendorBill, Requisition, RequisitionLine, RMA
)
...
class VendorAccountSerializer(TenantModelSerializer):
    """1:1 with Vendor — billing relationship fields."""
...
class VendorAccountViewSet(TenantNoDeleteViewSet):
    """Read/update only — VendorAccounts are auto-created with Vendor and
    deleted only via Vendor cascade."""

    queryset = VendorAccount.objects.all()
    serializer_class = VendorAccountSerializer
    filterset_fields = ['vendor_id', 'credit_status', 'tax_exempt', 'po_required']
    search_fields = ['vendor__name', 'vendor__vendor_number',
...
class VendorSerializer(TenantModelSerializer):
    account = VendorAccountSerializer(read_only=True)
...
class VendorViewSet(TenantModelViewSet):
    queryset = Vendor.objects.select_related('account').all()
...
router.register(r'vendor-accounts', VendorAccountViewSet, basename='vendor-account')
